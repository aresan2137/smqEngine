...
#[allow(dead_code)]
impl Material {
    pub fn from_json(context: &Context, json: &str, code: &str, layout: &[&BindGroupLayout]) -> Self {
        let json_data: MaterialJson = serde_json::from_str(json).expect("failed to parse material json");
        
        let mut attachments = Vec::new();

        for (_i, attachment_edit) in json_data.atachments.iter().enumerate() {
            let format = json_helper::str_to_texture_format(&attachment_edit);
