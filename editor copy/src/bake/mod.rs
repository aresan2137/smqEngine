...
//use crate::editor::{EditorState, inspector::{RenderTextureJson, TextureJson, WgslJson, process_common_file_descriptor}};

pub fn bake(_state: &EditorState) -> Result<(), Box<dyn Error>> { Ok(()) }
...
//             analisys.0.push(analize_wgsl_file(&file_path, &mut validator)?);
//             analisys.1.push(path.clone());
//         }
//     }

//     bake_auto_ubos(&analisys.0, &mut struct_code, &mut fn_code, &mut ret_code, &mut ret_struct_code)?;
