        ...
        if entry.path().is_file() {
            ...
            match ext {
                ...
                "renderTexture" => {
                    ...
                    let json_content = fs::read_to_string(&json_path)?;
                    let render_texture_json: RenderTextureJson = serde_json::from_str(&json_content).map_err(|e| Error::new(ErrorKind::InvalidData, e))?;

// smq_engine::units::render_texture::RenderTexture
// pub fn new(
//  context: &Context,
//  width: u32,
//  height: u32,
//  attachments_data: &[(TextureFormat, TextureUsages, Option<&str>)],
//  depth_data: Option<(TextureFormat, TextureUsages, Option<&str>)>
// ) -> Self

// Go to Option | Context | RenderTexture | TextureFormat | TextureUsages
